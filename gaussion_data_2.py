...
if __name__ == "__main__":
    ...
    radius = 3
    mean = np.array([[radius * math.cos(2 * np.pi * idx / float(n_components)),
                      radius * math.sin(2 * np.pi * idx / float(n_components))] for idx in range(n_components)])
    cov = np.array([cal_cov(2 * np.pi * idx / float(n_components)) for idx in range(n_components)])
    ys = GMM_sampler(N=20000, mean=mean, cov=cov)

    X_train, X_val, X_test = ys.X_train, ys.X_val, ys.X_test

    # dataset = torch.Tensor(X_train).float()
    # dataloader1 = torch.utils.data.DataLoader(dataset, batch_size=128, shuffle=True)
    ...
    for i, frame in enumerate(frames):
        ...
        #
        # # 绘制测试集数据点，根据密度设置颜色深浅
        # scatter = ax.scatter(frames[:, 0], frames[:, 1], c=z, cmap='cool', s=5)
        #
        # # 设置颜色条
