...
ckpt = torch.load('ckpt0.pth', map_location=device)
...
test_dataloader = torch.utils.data.DataLoader(dataset_false, batch_size=64, shuffle=True)
...
# 保存到Excel文件时，分别指定列名
df.to_excel('results_tab1_false1.xlsx', index=False, columns=['Prior_LogP', 'Later_LogP'])
