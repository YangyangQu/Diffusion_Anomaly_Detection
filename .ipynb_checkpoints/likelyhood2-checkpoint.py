...
parser.add_argument('--ckpt_path', type=str, default='ckpt1_9.pth', help='Path to the checkpoint file')
...
# 创建数据加载器
data_loader_0_labels = DataLoader(dataset_label_0, batch_size=batch_size, shuffle=True, num_workers=4)

# 在代码的开始处初始化一个空的DataFrame
df = pd.DataFrame(columns=['Label', 'Prior_LogP', 'Later_LogP', 'Prior_LogP1', 'Later_LogP1'])

# 创建保存图像的文件夹
save_dir = f'{results_dir}/sample_images19_using1_resnet_early_stop'
os.makedirs(save_dir, exist_ok=True)

# 加载预训练模型
score_model = torch.nn.DataParallel(ShallowResNet(marginal_prob_std=functools.partial(marginal_prob_std, sigma=sigma)))
score_model = score_model.to(device)
ckpt = torch.load(ckpt_path, map_location=device)
score_model.load_state_dict(ckpt)

# 数据处理和模型推断部分
tqdm_data = tqdm(data_loader_0_labels, desc="Processing")
k = 0
for x, batch_label in tqdm_data:
    ...
    for i in range(x.size(0)):
        ...
        z, prior_logp, later_lopg = ode_likelihood(x_sample, score_model, functools.partial(marginal_prob_std, sigma=sigma),
                                                   functools.partial(diffusion_coeff, sigma=sigma),
...
# 保存到Excel文件时，分别指定列名
results_filename = f'{results_dir}/results.xlsx'
