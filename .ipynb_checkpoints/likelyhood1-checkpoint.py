...
# 创建数据加载器
data_loader_other_labels = DataLoader(dataset_other_labels, batch_size=batch_size, shuffle=True, num_workers=4)

# 在代码的开始处初始化一个空的DataFrame
df = pd.DataFrame(columns=['Label', 'Prior_LogP', 'Later_LogP', 'Prior_LogP1', 'Later_LogP1'])

# 创建保存图像的文件夹
save_dir = f'{results_dir}/sample_images19_using1_z'
...
for x, batch_label in tqdm_data:
    ...
    for i in range(x.size(0)):
        ...
        x_ode_sample = x_sample.cpu().numpy()
        # x_ode_sample = (x_ode_sample * 255. + np.random.rand(*x_ode_sample.shape)) / 256
        x_ode_sample = torch.from_numpy(x_ode_sample).to(device)
