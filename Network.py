...
class Block(nn.Module):
    def __init__(self, size: int, device='cuda'):
        ...
        self.ff = nn.Linear(size, size).to(device)
        self.act = nn.GELU().to(device)

    def forward(self, x: torch.Tensor):
        return x + self.act(self.ff(x))

device = 'cuda'  # @param ['cuda', 'cpu'] {'type':'string'}
class ScoreNetMLP1(nn.Module):
    def __init__(self, hidden_size: int = 128, hidden_layers: int = 3, embed_dim: int = 128,
                 marginal_prob_std=None):
        super().__init__()

        # 使用 GaussianFourierProjection 替换原来的 PositionalEmbedding
        self.embed = nn.Sequential(GaussianFourierProjection(embed_dim=embed_dim),
                                   nn.Linear(embed_dim, embed_dim))

        concat_size = embed_dim  # emb_size，因为只有一个输入嵌入
        layers = [nn.Linear(concat_size + 2, hidden_size), nn.GELU()]  # 加 1 表示拼接 t
        for _ in range(hidden_layers):
            layers.append(Block(hidden_size))
        layers.append(nn.Linear(hidden_size, 2))
        self.joint_mlp = nn.Sequential(*layers)

        self.marginal_prob_std = marginal_prob_std  # Assuming marginal_prob_std is a function you've defined
        self.act = lambda x: x * torch.sigmoid(x)

    # def forward(self, x, t):
    #     # 使用 GaussianFourierProjection 进行输入嵌入
    #     x_emb = self.act(self.embed(t)).to(device)
    #     print("x___ size:", x_emb.size())
    #     print("x size:", x.size())
    #
    #     # 将 x_emb 和 t 进行拼接
    #     x = torch.cat([x_emb, t.unsqueeze(-1)], dim=-1)
    #     x = self.joint_mlp(x)
    #     return x
    ...
    def forward(self, x, t):
        ...
        x = x.to(device)
        # 使用 GaussianFourierProjection 进行输入嵌入
        emb = self.act(self.embed(t)).to(device)
