...
class GMM_sampler(object):
    def __init__(self, N, mean=None, n_components=None, cov=None, sd=None, dim=None, weights=None):
        ...
        if mean is None:
            ...
            self.X = np.array([np.random.multivariate_normal(mean=self.mean[i], cov=self.cov[i]) for i in self.Y],
                              dtype='float64')
...
class Multi_dis_sampler(object):
    def __init__(self, N, dim):
        ...
        self.X_gmm = np.array([np.random.multivariate_normal(mean=self.mean[i], cov=self.cov[i]) for i in comp_idx],
                              dtype='float64')
        # dim 3 is a normal
    ...
    def get_single_density(self, data):
        ...
        for i in range(4, self.dim):
            p1 = 1. / (np.sqrt(2 * np.pi) * self.sd[0]) * np.exp(
                -np.sum((self.centers[0] - data[i]) ** 2) / (2 * self.sd[0] ** 2))
            p2 = 1. / (np.sqrt(2 * np.pi) * self.sd[1]) * np.exp(
                -np.sum((self.centers[1] - data[i]) ** 2) / (2 * self.sd[1] ** 2))
            p_indep_gmm *= (p1 + p2) / 2.
...
class Mixture_sampler_v2(object):
    def __init__(self, nb_classes, N, dim, weights=None, sd=0.5):
        ...
        if nb_classes <= dim:
            ...
            if dim == 2:
                self.mean = np.array([(np.cos(2 * np.pi * idx / float(self.nb_classes)),
                                       np.sin(2 * np.pi * idx / float(self.nb_classes))) for idx in
                                      range(self.nb_classes)])
            else:
                self.mean = np.zeros((nb_classes, dim))
                self.mean[:, :2] = np.array([(np.cos(2 * np.pi * idx / float(self.nb_classes)),
                                              np.sin(2 * np.pi * idx / float(self.nb_classes))) for idx in
                                             range(self.nb_classes)])
        ...
        self.X_c = np.array([np.random.multivariate_normal(mean=self.mean[i], cov=self.cov[i]) for i in self.Y],
                            dtype='float64')
    ...
    def predict_onepoint(self, array):  # return component index with max likelyhood
        ...
        return np.argmax(
            [multivariate_normal.pdf(array, self.mean[idx], self.cov[idx]) for idx in range(self.nb_classes)])
