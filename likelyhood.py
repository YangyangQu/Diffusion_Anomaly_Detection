...
device = 'cuda'  # @param ['cuda', 'cpu'] {'type':'string'}
ckpt = torch.load('model_n1200_lr0.0001_sigma25.0.pth', map_location=device)
...
# 保存到Excel文件时，分别指定列名
df.to_excel('results111.xlsx', index=False, columns=['Label', 'Prior_LogP', 'Later_LogP', 'Prior_LogP1', 'Later_LogP1'])
