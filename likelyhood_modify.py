...
# 计算10%的数据集大小
subset_size = int(len(dataset_other_labels) * 0.05)

# 随机划分数据集，获取10%的子集
subset_dataset = random_split(dataset_other_labels, [subset_size, len(dataset_other_labels) - subset_size])[0]

# 创建数据加载器
dataset_label_0 = DataLoader(dataset_label_0, batch_size=batch_size, shuffle=True, num_workers=4)
dataset_label_others = DataLoader(subset_dataset, batch_size=batch_size, shuffle=True, num_workers=4)
# 在代码的开始处初始化一个空的DataFrame
# 在代码的开始处初始化一个空的DataFrame
df = pd.DataFrame(columns=['Label', 'Prior_LogP_0', 'Later_LogP_0', 'Prior_LogP_1', 'Later_LogP_1',
                           'Prior_LogP_2', 'Later_LogP_2', 'Prior_LogP_3', 'Later_LogP_3',
                           'Prior_LogP_4', 'Later_LogP_4', 'Prior_LogP_5', 'Later_LogP_5',
                           'Prior_LogP_6', 'Later_LogP_6', 'Prior_LogP_7', 'Later_LogP_7',
                           'Prior_LogP_8', 'Later_LogP_8', 'Prior_LogP_9', 'Later_LogP_9'])

# 创建保存图像的文件夹
save_dir = f'{results_dir}/sample_images19_sigma{sigma}_n_epochs_{n_epochs}'
...
# 数据处理和模型推断部分
tqdm_data = tqdm(dataset_label_others, desc="Processing")
k = 0
for x, batch_label in tqdm_data:
    ...
    for i in range(x.size(0)):
        ...
        # z_x_sample = z_x_sample* 0.7 + z_tmp* 0.3
        ...
        # 根据标签选择不同的列
        df.loc[len(df)] = [label] + [None] * (label * 2) + [prior_logp.item(), later_lopg.item(), prior_logp1.item(), later_lopg1.item()]
...
# 保存到Excel文件时，分别指定列名
results_filename = f'{results_dir}/resultsUsing19_{n_epochs}_{sigma}.xlsx'
